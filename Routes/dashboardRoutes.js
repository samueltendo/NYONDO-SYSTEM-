...
router.get("/", ensureAuthenticated, ensureRole("admin"), async (req, res) => {
  try {
    ...
    const totalDebt = allCredit.reduce(
    ...
    const notifications = [
      ...recentSales.map((s) => ({
        staffRole: "Sales",
        message: `Sold ${s.itemName}`,
        time: "Recent",
        customer: s.customerName,
      })),
      ...allDeposits.slice(-2).map((d) => ({
        staffRole: "Accounts",
    ...
    const modules = [
      {
        ...
        desc: "Track hardware debts",
        link: "/credit",
      },
      {
        name: "Deposit Scheme",
        desc: "Manage customer savings",
        link: "/deposits",
      },
      {
        name: "Financial Reports",
        desc: "P&L and Audit logs",
        link: "/reports",
      },
      {
